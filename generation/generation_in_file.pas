...
//--------rigth----------------
  for i:= 1 to size 
  do for j:= 1 to size
  do if (kakuro[i,j].btype=1) or (kakuro[i,j].btype=0) then begin
    count:=0;
    m:=j+1;
    ...
    if count>9 then begin     
      kakuro[i,j+1].btype:=1;
      kakuro[size+2-i,size+2-(j+1)].btype:=1; 
      end;
  end;
...
//--------down----------------
   for j:= 1 to size do for i:= 1 to size do if (kakuro[i,j].btype=1) or (kakuro[i,j].btype=0) then begin
    ...
    if count>9 then begin kakuro[i+1,j].btype:=1;kakuro[size+2-(i+1),size+2-j].btype:=1; end;
  end;
//------------------------
end;
...
begin
  ...
  if (size - 1) mod 2 = 1 then begin
    inc(mirrow);
    for j := 2 to (size div 2)+(size mod 2)+1 do 
    begin
...
begin      
    ...
    if (kakuro[i,j].btype=2) then begin    
      ...
      repeat
      q:=true;
      for n:=1 to 9 do if not (n in s) then q:=false; 
         if not(q) then kakuro[i,j].bvalue2:=random(9)+1 else begin   
            kakuro[i,j].btype:=1;
            q:=true;
         end;
